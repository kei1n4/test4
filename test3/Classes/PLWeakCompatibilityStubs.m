...
static PLObjectPtr PLLoadWeakRetained(PLObjectPtr *location) {
    /* Hand off to MAZWR */
    if (has_mazwr()) {
        MAZeroingWeakRef *mazrw = ( MAZeroingWeakRef *) *location;
...
static void SwizzledReleaseIMP(PLObjectPtr self, SEL _cmd) {
    ...
    Class lastSent = ( Class)CFDictionaryGetValue(tls->lastReleaseClassTable, self);
    ...
    CFDictionarySetValue(tls->lastReleaseClassTable, self, ( void *)targetClass);
...
static void SwizzledDeallocIMP(PLObjectPtr self, SEL _cmd) {
    ...
    Class lastSent = ( Class)CFDictionaryGetValue(tls->lastDeallocClassTable, self);
    Class targetClass = lastSent == Nil ? object_getClass(self) : class_getSuperclass(lastSent);
    targetClass = TopClassImplementingMethod(targetClass, deallocSELSwizzled);
    CFDictionarySetValue(tls->lastDeallocClassTable, self, ( void *)targetClass);
...
static void EnsureDeallocationTrigger(PLObjectPtr obj) {
    ...
    if (CFSetContainsValue(gSwizzledClasses, ( const void *)c))
        return;
    ...
    CFSetAddValue(gSwizzledClasses, ( const void *)c);
}
